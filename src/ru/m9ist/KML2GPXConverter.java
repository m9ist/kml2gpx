...
public class KML2GPXConverter {
    ...
    /**
     * Получает рандомную строку длиной 5 символов из [A-Z0-9]
     */
    ...
    /**
     * @param tmpDir   временная директория для работы парсера (ВНИМАНИЕ, в директорию делается распаковка,
     *                 а потом полная очистка, поэтому директория действительно должна быть временной, а не абы какой)
     * @param outDir   выходная директория, чтобы туда ложить обработанные файлы (имена файлов будут сохранены по
     *                 оригинальному имени .kmz файла с добавлением рандомной строки, чтобы не было проблем с
     *                 пересечением имен)
     * @param achiever путь до архиватора 7-zip //todo переделать через интерфейс архиватора, чтобы передавать его
     */
    public KML2GPXConverter(final File tmpDir, final File outDir, final String achiever) {
        this.tmpDir = tmpDir;
        this.outDir = outDir;
    ...
    /**
     * Производит парсинг указанного файла .kmz и преобразование его в .gpx формат
     *
     * @param source .kmz файл
     * @return строка, в которую положено содержимое в .gpx формате
     */
    public String parseKML(final File source) {
    ...
    private void processFile(final File file) {
        ...
        final String out = parseKML(kmlFile);
    ...
    public static void main(final String[] args) {
        ...
        final KML2GPXConverter KML2GPXConverter = new KML2GPXConverter(tmpDir, outDir, achiever);
        KML2GPXConverter.clearDir(tmpDir);
        System.out.println("Start processing " + input.getAbsolutePath());
        if (input.isFile()) {
            KML2GPXConverter.processFile(input);
        } else if (input.isDirectory()) {
            KML2GPXConverter.processDir(input);
        }
        KML2GPXConverter.clearDir(tmpDir);
        tmpDir.delete();
